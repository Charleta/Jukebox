...
if exist "%CHROME%" (
  start "" "%CHROME%" --user-data-dir="%YOUTUBE_PROFILE%" --new-window --start-fullscreen --autoplay-policy=no-user-gesture-required --no-first-run --disable-session-crashed-bubble --disable-infobars "%YOUTUBE_URL%"
) else (
  start "" "%YOUTUBE_URL%"
)
